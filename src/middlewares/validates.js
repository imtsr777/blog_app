const registerValidator = (req,res,next) =>{
    const {firstname,lastname,password,phone,bestCategory,username} = req.body
    try{
        ...
        if(!username) throw new Error("Username must")
        ...
        if(typeof(lastname) !== 'string' || lastname.length>15 || lastname.length<3){
            throw new Error("Lastname must be string max length 14 min length 5")
        }
        ...
        if(typeof(username) !== 'string' || username.length>20 || username.length<6){
            throw new Error("Username must be min 6 max 20 length")
        }
    }
